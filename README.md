...
It provides a set of generic primitives and a `useDragAndDrop` hook for
building reorderable lists and cross-container interactions without coupling
the drag-and-drop logic to a specific domain model.

The library handles drag state, drop positioning, reordering, cross-container
movement, and visual drop indicators, while leaving the data structure and
...
- Domain-agnostic container and item models
...
| `DndContainerLike` | `{ id: string }` | The only requirement on *your* container type: it needs an `id`. Everything else about its shape (`title`, `cards`, whatever) is yours. |
...
| `DndContainerView<C, T>` | `C & { slots: DndSlot<T>[]; isDragOver: boolean }` | *Your* container type, augmented with everything needed to render it during a drag. |
...
function TaskBoard({ initial }: { initial: TaskList[] }) {
  const { containers, getItemHandlers, getContainerHandlers } = useDragAndDrop(initial, {
...
```css
.dnd-dragging { display: none; }               /* hides the item mid-drag */
.dnd-drop-indicator { /* ghost placeholder look, e.g. */ border: 2px dashed; }
```
